from flask import request, jsonify, current_app, redirect, url_for, send_file
...
def init_auth_routes(app):
    ...
    def login():
        try:
            ...
            email = data.get('email')
            password = data.get('password')
            otp = data.get('otp')  # optional MFA token
            ...
            # Case 1: MFA not set up yet (first login)
            if not user.mfa_enabled:
                # Generate a new secret if not already assigned
                ...
                return jsonify({
                    ...
                    'message': 'MFA setup required. Scan this QR code with Google Authenticator.',
                    'otp_uri': otp_uri,
                    'secret': user.mfa_secret
                }), 200

            # ---------------- MFA Verification (if already set up) ----------------
            if user.mfa_enabled:
                if not otp:
                    return jsonify({
                        'mfa_required': True,
                        'message': 'MFA enabled. Please provide your OTP code.'
                    }), 401
    ...
    # ---------------- VERIFY MFA ----------------
    @app.route('/api/auth/verify_mfa', methods=['POST'])
    def verify_mfa():
        try:
            data = request.get_json()
            email = data.get("email")
            ...
            if not user:
                current_app.logger.warning(f"MFA verification failed: User not found ({email})")
                return jsonify({"error": "User not found"}), 404

            # ---------------- OTP Verification ----------------
            import pyotp  # Ensure pyotp is imported
            if not user.mfa_secret:
                current_app.logger.warning(f"MFA verification failed: User {email} has no MFA secret")
                return jsonify({"error": "MFA not setup for user"}), 400

            totp = pyotp.TOTP(user.mfa_secret)
            if totp.verify(otp, valid_window=1):  # Accepts ±1 time step
                tokens = AuthService.generate_tokens(user)
                AuditService.log(user_id=user.id, action="login_mfa_verified")
                current_app.logger.info(f"MFA verified successfully for user {email}")
                return jsonify({
                    "success": True,
            ...
            current_app.logger.error(f"MFA verification error: {str(e)}", exc_info=True)
            return jsonify({"error": "Internal server error"}), 500



    # ---------------- QR CODE IMAGE ----------------
    @app.route('/api/auth/mfa_qr/<email>', methods=['GET'])
    def generate_mfa_qr(email):
        try:
            user = User.query.filter(db.func.lower(User.email) == email.lower()).first()
            if not user or not user.mfa_secret:
                return jsonify({'error': 'User or MFA secret not found'}), 404
            ...
            current_app.logger.error(f"MFA QR generation error: {str(e)}", exc_info=True)
            return jsonify({'error': 'Internal server error'}), 500

    ...
    def _merge_candidate_profile(candidate: Candidate, external_result: dict):
        ...
        def _norm_skill(item):
            ...
            if isinstance(item, dict):
                return (item.get('skill') or item.get('name') or item.get('value')), float(item.get('confidence', 1.0) or 0.0)
            return str(item), 1.0
        ...
        if external_education:
            ...
            for edu in _as_list(external_education):
                if isinstance(edu, dict):
                    if float(edu.get('confidence', 1.0) or 0.0) <= 0.9:
                        continue
                    degree = (edu.get('degree') or '').strip()
                    institution = (edu.get('institution') or '').strip()
                    if degree or institution:
                        new_items.append({"degree": degree or None, "institution": institution or None})
                else:
                    s = str(edu).strip()
                    if s:
        ...
        if external_experience:
            ...
            for exp in _as_list(external_experience):
                if isinstance(exp, dict):
                    if float(exp.get('confidence', 1.0) or 0.0) <= 0.9:
                        continue
                    position = (exp.get('position') or exp.get('title') or '').strip()
                    company = (exp.get('company') or '').strip()
                    if position or company:
                        new_items.append({"title": position or None, "company": company or None})
                else:
                    s = str(exp).strip()
                    if s:
                        new_items.append({"value": s})

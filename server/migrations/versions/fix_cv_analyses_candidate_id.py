...
def upgrade():
    # Move cv_analyses to cv_analyser schema if it's still in public
    conn = op.get_bind()
    inspector = sa.inspect(conn)
    public_tables = inspector.get_table_names(schema='public')
    cv_analyser_tables = inspector.get_table_names(schema='cv_analyser')
    
    if 'cv_analyses' in public_tables and 'cv_analyses' not in cv_analyser_tables:
        op.execute("ALTER TABLE public.cv_analyses SET SCHEMA cv_analyser")
        print("Moved cv_analyses from public to cv_analyser schema")

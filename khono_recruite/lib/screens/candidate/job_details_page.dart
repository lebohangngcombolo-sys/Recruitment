...
class _JobDetailsPageState extends State<JobDetailsPage> {
  ...
  Future<void> loadDraft() async {
    ...
    try {
      final res = await http.get(
        Uri.parse("${ApiEndpoints.candidateBase}/applications/$applicationId/draft"),
        headers: {
  ...
  Future<void> applyJob() async {
    ...
    try {
      final res = await http.post(
        Uri.parse(localhostToEnv("http://127.0.0.1:5000/api/candidate/apply/${widget.job['id']}")),
        headers: {
          "Content-Type": "application/json",
          "Authorization": "Bearer $token",
        },
  ...
  String _formatSalary(Map<String, dynamic> job) {
    ...
    if (min == null && max == null) {
      final rangeStr = job['salary_range']?.toString().trim() ?? job['salary']?.toString().trim();
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Image.asset(
            ...
            fit: BoxFit.contain,
            color: _textPrimary,
            errorBuilder: (_, __, ___) => Icon(Icons.link, size: 20, color: _textPrimary),
          ),
        ),
      ),
    );
